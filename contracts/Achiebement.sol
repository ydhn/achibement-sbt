...
contract Achiebement is ERC1155, ERC1155Burnable, Ownable, ERC5633 {
    ...
    function recover(uint256[] calldata ids, uint256[] calldata amounts, bytes memory _signature)
    private
    view
    returns (address)
    {
        return ECDSA.recover(ECDSA.toEthSignedMessageHash(keccak256(abi.encode(msg.sender, ids, amounts))), _signature);
    }

    function mint(uint256[] calldata ids, uint256[] calldata amounts, bytes memory _signature)
    external
    {
        require(ids.length == amounts.length, "Invalid arguments");
        require(_signer == recover(ids, amounts, _signature), "Invalid Signature");
